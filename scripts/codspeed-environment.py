...
one level up, since a new stable rustc emits different code, and the C library
is a third; all three are recorded here and all three are compared.
...
def parse_cpuinfo(text):
    ...
    cores = {
        (field(block, "physical id"), field(block, "core id")) for block in blocks
    }
    cores.discard(("", ""))
    ...
    brand = field(first, "model name") or " ".join(
        field(first, name)
        for name in ("CPU part", "CPU variant", "CPU revision")
    ).strip()
...
def _darwin_cpu():
    ...
    flags = " ".join(
        sysctl(name)
        for name in ("machdep.cpu.features", "hw.optional.arm.caps")
    )
    return {
