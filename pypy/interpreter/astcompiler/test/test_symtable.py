...
def f(x):
    ...
    def test_annotation_global4(self):
        src_global = ("class A:\n"
                      "    global x\n"
                      "    x: int\n")
        exc_global = py.test.raises(SyntaxError, self.func_scope, src_global).value
    ...
    def test_named_expr_list_comprehension(self):
        # PEP 709: listcomp is inlined, so the walrus target is a plain local
        # of the enclosing function (no longer a cell for the nested scope).
