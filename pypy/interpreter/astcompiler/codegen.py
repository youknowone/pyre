...
class PythonCodeGenerator(assemble.PythonCodeMaker):
    ...
    def _restore_inlined_comp_locals(self, indices):
        ...
        while i >= 0:
            name, kind = indices[i]
            if kind == 'cell':
                self.emit_op_arg(ops.STORE_CLOSURE, self.cell_vars[name])
            else:
                self.emit_op_arg(ops.STORE_FAST, self.var_names[name])
            i -= 1
        ...
        for name, scope in comp_scope.symbols.iteritems():
            ...
            if is_bound or in_class:
                ...
                if scope == symtable.SCOPE_CELL:
                    index = self.add_name(self.cell_vars, name)
                    self.emit_op_arg(ops.LOAD_CLOSURE, index)
                    self.emit_op_arg(ops.MAKE_CELL, index)
                    indices.append((name, 'cell'))
                else:
                    index = self.add_name(self.var_names, name)
                    self.emit_op_arg(ops.LOAD_FAST_AND_CLEAR, index)
                    indices.append((name, 'fast'))
        n = len(indices)
