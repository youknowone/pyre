...
pyre records traces through 72 `try_walker_specialize_*` functions — 68 in
`jitcode_dispatch/specialize.rs`, one in `residual_call.rs`
(`load_deref`) and three in `inline_call.rs` (`instance_iter`,
`instance_next`, `generator_next`) — described by the 82 rows of
...
overstates it by five; the fold count is 77. The three unary descent rows
...
* Rows — select the complete `spec_folds!` invocation by its symbol boundaries:
  ...
  answers 82; replacing the final matcher with `rg -cF '_descent"'` answers
...
* Definitions — `rg -c` reports one count *per file*, so it answers 68/1/3
...
is 77 hand-written rows plus 5 orthodox descent rows, re-derived on 2026-09-26.
...
prefix leaf. A census over the whole corpus as it stood
...
and they name 57 distinct rows between them (3 of those descent rows;
`subscr_tuple_descent` and `load_super_attr_descent` are named by none), so
25 of the 82 rows have no fixture coupling at all
