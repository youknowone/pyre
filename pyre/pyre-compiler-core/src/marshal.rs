// Based line-for-line on RustPython compiler-core at pyre's pinned
// 1613741f861b4f289da59ffc78a20ead4b4444fc revision.  Local differences are
// limited to `MarshalBag` placeholder hooks and the matching CPython-style
// container construction branches below.
...
fn deserialize_code_inner<R: Read, Bag: ConstantBag>(
    ...
    let instructions =
        CodeUnits::try_from(code_bytes.as_slice()).map_err(|_| MarshalError::InvalidBytecode)?;
...
pub trait MarshalBag: Copy {
    ...
    /// CPython `r_object()` installs mutable and partially-built container
    /// objects in `refs` before reading their children.  Runtime bags which
    /// can expose such an object opt in through these construction hooks;
    /// constant bags retain the upstream collect-then-construct path.
