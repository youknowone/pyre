...
pub fn run(module_path: &Path, source: &str, script: &Path) -> Result<i32, String> {
    ...
    let env_names = instance
        .get_typed_func::<(), u64>(&store, "pyre_env_names")
        .ok();
    let set_env = instance
        .get_typed_func::<(u32, u32), ()>(&store, "pyre_set_env")
        .ok();
