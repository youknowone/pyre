...
def run():
    ...
    # frame and at one line. Bindings stay local: a module-level `except E as e`
    # is STORE_NAME + DELETE_NAME every iteration and revokes function-entry
    # traces that folded a LOAD_GLOBAL from this module
    # (`celldict.py notify_version_watchers`).
    same_frame_tb = 0
    tb_linenos = set()
    tb_frame_is_self = 0
    for i in range(N):
