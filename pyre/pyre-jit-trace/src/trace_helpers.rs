//! Fixed trace-helper code for `MIFrame` — the typed trace operations,
//! their concrete-computation helpers, the unbox/box/binop trace
//! primitives, and the operator dispatch tables.
//!
//! These are hand-maintained, compiler-checked Rust — the trace-time
//! analogs of PyPy's hand-written RPython: the operator dispatch tables
//! mirror `resoperation.py` / `lloperation.py` data; the concrete
//! computation helpers mirror `executor.py` + `floatobject.py` /
//! `intobject.py`; the unbox/box/binop primitives and the typed
//! `generated_*` operations mirror `pyjitpl.py`'s `opimpl_*` and
//! `_record_helper`. None of this is PyPy-generated; it is real code, so
//! it lives in a real module rather than being emitted as string
//! literals by the translator.
