impl pyre_interpreter::ControlFlowOpcodeHandler for crate::state::MIFrame {
    ...
    fn close_loop_args(
        ...
        self.with_ctx(|this, ctx| {
            ...
            // tracked the live depth back to the loop-header invariant by
            // this back-edge; sync it into the snapshot so
            // `close_loop_args_at`'s `concrete_valuestackdepth()` read carries
            // the merge-point depth instead of the stale seed (a stale
            // over-count emits phantom stack operands into the closing JUMP,
            // corrupting the re-entered loop frame).  No-op when the snapshot
            // is already live (`concrete == symbolic`).
