...
fn try_walker_inline_resolved_user_call_inner<Sym: WalkSym>(
    ...
    if !guards_the_callee_function {
        ...
        // loop-invariant.
        //
        // Keep GETFIELD+GUARD_VALUE even when the callable is a ConstPtr.
        // The `code?` / `w_func_globals?` marker matches `function.py
        // getcode()` but is the `test_uuid` SIGSEGV: after `test_UUID`
        // warms the JIT, `jit_bigint_int_eq` receives a small int (2) as
        // `*const BigInt`. Until that residual is sound under the marker,
        // the live field read stays.
