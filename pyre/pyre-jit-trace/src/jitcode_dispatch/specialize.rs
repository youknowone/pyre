...
/// `s.add(x)` written as a method call: fold the bound method away and emit
/// the store as one residual, the shape `list.append` already has.
///
/// Measured against pypy3 7.3.20 with the empty loop subtracted, `set.add` was
/// 15.8x where `list.append` is 2.2x — and the discriminator is inside one
/// engine, not between them: pypy keeps add and append within 1.35x of each
/// other, pyre spread them 9.7x.  The loop body carried **two**
/// `CallMayForceR` — `bh_load_attr_fn` to look `add` up and build a bound
/// method, then `bh_call_fn` to invoke it — where pypy's carries one
/// `guard_nonnull_class(W_SetObject)` and one call.  This removes the first of
/// the two: the attribute lookup and the method allocation fold into the same
/// `guard_class` + `guard_value` pair the append arm uses, and the receiver
/// comes out of the method's own `w_self` field.
///
/// The store itself stays a residual, and that is upstream's shape too, not a
/// concession: pyre's set storage is an `IndexMap` with no lowering, and
/// `rordereddict.py _ll_dict_setitem_lookup_done` is
/// `@jit.look_inside_iff(jit.isvirtual(d) and jit.isconstant(key))` — neither
/// conjunct can hold here.  So there is no body worth descending, and the
/// `w_list_append_inner` sub-walk this arm otherwise mirrors has no set twin.
///
/// It stays `may_force` as well.  Upstream's counterpart residual records
/// `EF_CAN_RAISE`, not `EF_FORCES_VIRTUAL_OR_VIRTUALIZABLE`, so the forcing is
/// the second half of the gap — but a `set.add` element's `__hash__` can run
/// Python that reads the frame, and until `vable_token` is published before
/// compiled code owns the frame rather than one iteration after it, the lazy
/// force is what would be skipped.  Downgrading the effect belongs with that
/// fix, not here.
///
/// Recognition declines before anything is emitted, so a subclass receiver, a
/// rebound `add`, or a non-`set` falls through to the generic residual.
pub(crate) fn try_walker_orthodox_set_add<Sym: WalkSym>(
    ctx: &mut WalkContext<'_, '_, Sym>,
    code: &[u8],
    op: &DecodedOp,
    r_args: &[OpRef],
...
fn try_walker_orthodox_set_method<Sym: WalkSym>(
    ...
    let (inner_func, inner_self) = unsafe {
        ...
        // Recognition has to establish exactly what the `guard_exact_w_class`
        // below pins, which is `w_class`.  An `ob_type` test is the wrong
        // spelling rather than a weaker one: a subclass instance shares the
        // payload type with `w_class` overwritten, so it would be admitted here
        // and then fail that guard on every execution without ever converging.
        // `frozenset` carries neither method, and a subclass may rebind them or
        // carry a `__hash__` the shared body would bypass.
    ...
    // Tracing is execution (`orthodox_list_append_commit`).  Apply the
    ...
    let none_const = ctx.trace_ctx.const_ref(none_obj as i64);
    ctx.trace_ctx.set_opref_concrete(
        none_const,
        majit_ir::Value::Ref(majit_ir::GcRef(none_obj as usize)),
    );
    write_residual_call_result_to_dst(ctx, op.pc, dst, 'r', none_const)?;
    Ok(Some(()))
}
