...
///     a bigint quotient, boxed as a `W_LongObject`.
...
/// True-divide, pow and any non-`W_LongObject` operand return `Ok(None)` so
/// the caller falls through to the generic record, preserving the `__op__`
/// semantics.
#[allow(clippy::too_many_arguments)]
