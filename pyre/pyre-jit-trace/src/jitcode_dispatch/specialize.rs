...
pub(crate) fn try_walker_specialize_load_super_attr<Sym: WalkSym>(
    ...
    // `w_method_new`.  The wall stays, but for a weaker reason than it used
    // to carry: the descent was quadratic in the iteration count, and is not
    // any more.  What keeps it here is that at this site it is still 4.2x the
    // fold, so firing it would move the site to the slower arm.
    //
    ...
    // output on every arm, best of 3, one script and one host.  Per
    // ITERATION, so a flat column is linear and a rising one is not:
    //
    ...
    //
    // "before" is `c827bea5aaa`, "after" is `7d57e55a2dc` with the root
    // bracket's close emitted.  The two binaries do not share a base, so the
    // fold and `PYRE_NO_JIT` columns are the free control: the base moved
    // them by at most 2.0x.  No base bump turns a rising column into a
    // falling one, and the descent's turns -- its total is now 0.049s fixed
    // plus 71ns per iteration, holding to within 0.5% across the four N.  With
    // that, "slower than not running the JIT at all" is gone: at
    // N=2,000,000 the descent is 7.4x FASTER than `PYRE_NO_JIT=1`, where it
    // was 6.8x slower.
    //
    // The cause was the GC root bracket, not the proxy allocation.  The
    // descended body pinned 10 roots per iteration and truncated none:
    ...
    // which `mir.rs`'s `TermKind::Drop => set_goto` arm erased when it
    // lowered the body (the `fn_ptr` naming the glue was discarded a layer
    // earlier, in `majit-charon-reader`'s `ullbc.rs` `Drop` variant).  The
    // bracket was push-only in jitcode, the stack grew by 10 slots per
    // iteration, and every collection walked all of it.  `PYPY_GC_NURSERY=512M`
    // moved N=2,000,000 from 18.2s to 10.0s and left the curve rising, which
    // is what a change to the NUMBER of collections does when each one still
    // walks a stack that keeps growing.
    //
    // That arm now lowers the one glue it can name as the call it is, and
    // the compiled body carries `shadow_stack_cell` x4,
    // `shadow_stack_cell_len` x4 and `shadow_stack_cell_truncate` x4 -- four
    // opens, four closes -- beside `pin_root` x10 and `shadow_stack_get` x10.
    // Residual calls print as raw addresses, so those are `nm` names,
    // recovered by solving for the dyld slide (the offset that lands the most
    // observed addresses on a symbol, restricted to multiples of 0x4000).
    //
    // It is not deopt, and never was: both arms report `loops_compiled=2
    // loops_aborted=0 bridges_compiled=0 guard_failures=1`, and
    // `MAJIT_GUARD_CENSUS` reads `distinct=1 total=1` on each -- one deopt
    // event across two million iterations either way.  `MAJIT_LOG_OPT` puts
    // the compiled body at 528 ops / 101 guards against the fold's 72 / 18,
    // and that body carries:
    //
    //   * 52 residual calls per iteration against the fold's zero, 36 of them
    //     `pyre_object::gc_roots` bookkeeping (`pin_root` x10,
    //     `shadow_stack_get` x10, `shadow_stack_cell` x4,
    //     `shadow_stack_cell_len` x4, `shadow_stack_len` x4,
    //     `shadow_stack_cell_truncate` x4).  They cannot be optimized out:
    //     `jtransform.rs` asserts in `gc_root_pin_and_reload_keep_their_calls`
    //     that `pin_root` and `reload_top_root` survive as calls, because the
    //     slot a pin publishes is what bound `shadow_stack_get` residuals
    //     read back.  Removing the bracket from a walked body altogether is
    //     the upstream shape -- upstream's codewriter never sees one, the
    //     transformer inserts roots downstream of it -- but it needs a proof
    //     that compiled code's own root discovery covers what the pins cover,
    ...
    //     Real, and never what made the curve rise: a per-iteration
    //     allocation is a constant per-iteration cost.
    ...
    // helper code that opens a root bracket, which is most of it -- and that
    // is why the bracket's close is emitted for every such body rather than
    // for this one.  What remains is a constant: upstream can trace
    // `W_Super.getattribute` because RPython's roots are invisible to the
    // tracer and its proxy virtualizes, and neither holds here.  The fold's
    // advantage is not that it is a better fold, it is that it never enters
    // helper code.
    //
...
/// Both operands of a `str` binary operation, with their concrete objects —
/// the shared gate of [`try_walker_specialize_compare_op_str`] and
...
/// `str` operands: the `str` analogue of
...
///
/// Tried after the int, long and float compares decline, so a numeric operand
...
/// Walker-native specialization for `str + str`, the concatenation arm of the
/// `BINARY_OP` residual.
///
/// `descr_add` (unicodeobject.py) builds one fresh `W_UnicodeObject` over the
/// joined WTF-8 payload, which `jit_str_concat` is.  Recording that call in
...
pub(crate) fn try_walker_specialize_binary_op_str<Sym: WalkSym>(
    ...
    // Authentic boxed result from the same may-force path the generic leg
    // uses; the recorded call reproduces it in compiled code.
    let Some(boxed_result_i64) = walker_execute_may_force_boxed(ctx, allboxes, call_descr) else {
        return Ok(None);
    };
    let boxed_result = boxed_result_i64 as pyre_object::PyObjectRef;
