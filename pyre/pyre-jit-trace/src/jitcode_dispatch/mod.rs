...
//!       (`pyjitpl.py` ABORT_ESCAPE parity). The `MIFrame` trait mirror
//!       survives only in test fixtures. `vrefs_after_residual_call`
//!       stays omitted: the walker tracks no `virtualref_boxes` and
//!       production `jit.virtual_ref` producers are absent (zero IR ops
