...
fn unary_neg_leaves_are_the_pypy_leaf() {
    for (path, arith) in [
        ...
        ("pyre_interpreter::objspace::descroperation::_float_sin", ""),
...
fn walk_return_value_helper_terminates_at_first_ref_return() {
    ...
    // caller's own `ref_return/r` at pc=9..11 then stashes the
    // outermost finish payload.
    ...
    assert!(
        tc.num_ops() > ops_before,
        "the callee sub-walk must have contributed at least one op",
    );
    // The outermost `ref_return/r` records no FINISH itself: it stashes the
    // payload for `full_body_walk_trace`, which builds the portal-exit FINISH.
...
/// A misspelt graph key matches nothing and silently turns its fold site
/// off, so every key the portal closure mints must resolve.
#[test]
fn intercepted_graph_keys_resolve() {
    use super::specialize::{
        FRAME_LOAD_ATTR_PATHS, GETATTR_STR_IMPL_PATH, SPACE_GETATTR_PATHS, SPACE_IS_TRUE_PATHS,
        SPACE_ITER_PATH, SPACE_SETITEM_PATHS,
    };
    let minted: [&[&str]; 8] = [
        SPACE_GETATTR_PATHS,
        &[GETATTR_STR_IMPL_PATH],
        FRAME_LOAD_ATTR_PATHS,
    ...
    for path in minted.iter().flat_map(|paths| paths.iter()) {
        assert!(
            crate::jitcode_runtime::compute_pathed_jitcode_index(path).is_some(),
            "no jitcode under graph key `{path}`"
        );
    }
}

/// The name `setitem` belongs to the dict strategies as well; only the
/// `space.setitem` graph may take the STORE_SUBSCR fold.
#[test]
fn dict_strategy_setitem_is_not_space_setitem() {
    let path = "dictmultiobject::ObjectDictStrategy::setitem";
    let index = crate::jitcode_runtime::compute_pathed_jitcode_index(path)
        .expect("dict strategy `setitem` graph");
    let body = sub_jitcode_body_by_index(index).expect("dict strategy `setitem` body");
    assert!(!super::specialize::jitcode_is_space_setitem(index, &body));
    assert!(!super::specialize::jitcode_is_unbounded_helper_body(index, &body));
}
