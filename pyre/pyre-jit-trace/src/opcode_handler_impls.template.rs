// `OpcodeHandler` trait impls for `MIFrame` — template form.
//
// `pyre/pyre-jit-trace/build.rs` reads this file and writes it verbatim to
// `OUT_DIR/jit_trace_trait_impls.rs`, which `lib.rs` then `include!`s once at
// crate root. The generated path goes through OUT_DIR (instead of being a
// regular `mod ...;` here) so that future phases of the eval-loop automation
// (per-opcode jitcode emission, MIFrame jitcode dispatch) can swap in
// codegen-derived content without disturbing the include path.
//
// **drift warning**: this file is currently a *transcription* of what the
// hand-written impls in trace_opcode.rs used to look like. helpers it calls
// (e.g. `MIFrame::push_value`, `crate::state::namespace_slot_direct`) live in
// the same crate; if you change those, you almost certainly need to change
// the matching impl block here too. `tests/trait_impls_snapshot.rs` guards
// against unintended drift by comparing against a checked-in snapshot.
