...
off that many of the 102 trace-time specialization rows (the `spec_folds!`
...
102 are hand-written: `subscr_tuple_descent`, `binary_op_descent`,
`compare_op_descent`, `builtin_len_descent` and `load_super_attr_descent`
name orthodox sub-walks of the
...
| `PYRE_JIT_CORE` | OFF | makes `scripts/extract-llbc.py` extract the fast JIT core: the interpreter's `full` modules are not rustc inputs, `pyre-module` is not extracted, and the artefacts go to `build/llbc-jit-core` so the product `build/llbc` set stays in place (`scripts/build-jit-core.sh` sets it); retire when the core is its own crate set with its own extraction list |
