...
| PYRE_WASM_INLINE_NONHEADER | admits an inlined region whose closing JUMP names a resumable LABEL other than the loop header (`lib.rs inline_nonheader_enabled`); `=1`/`true`/`on` arms it.  The corrupted-Ref miscompile that first held it off is root-caused and declined by `source_in_preamble`: the corpus is 437 pass + 1 jit-stats improvement armed, 438 pass unarmed.  It removes 49.4M of the 257.3M cross-module crossings on the 81 fixtures that reach the decline, but timed rather than counted it is 0.74x/0.67x on two fixtures, flat on most, and 1.23x on `spectral_norm` | an admission rule separates the fixtures it speeds up from `spectral_norm`, which sheds 99.7% of its crossings and still loses 23% |
