...
| `PYRE_CATCH_SCAN_DIAG` | OFF | prints, for every `find_catch_after_resume_live` call, how many startpoints the forward scan consumed before the trailing `-live-`, which is the distance #1151 records as unbounded; retire when the handler-search coordinates are the operation's own trailing `-live-` and the scan is gone |
