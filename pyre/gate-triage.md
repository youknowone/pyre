...
| PYRE_WASM_GC_REWRITE | runs `rewrite.py` (`GcRewriter::rewrite_for_gc_with_constants`) on every wasm `compile_loop` / `compile_bridge` op list (`lib.rs prepare_ops_for_compile`); `=1`/`true`/`on` arms it. Off keeps today's intern-only path byte-identical | when the wasm backend consumes rewritten traces as its only compile path |
