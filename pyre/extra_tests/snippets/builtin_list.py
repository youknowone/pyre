...
# A list stores the original object reference even when the payload is eligible
# for pyre's numeric unboxing optimizations.
for numeric_value in (int("257"), float("1.5")):
    direct = [numeric_value]
    assert direct[0] is numeric_value
    assert direct[0] is direct[0]
    appended = []
    appended.append(numeric_value)
    assert appended[0] is numeric_value


def hot_numeric_store_identity():
    ints = [int("257")]
    floats = [float("1.5")]
    for i in range(5000):
        int_value = i + 257
