...
fn init_sysconfigdata(ns: PyObjectRef) {
    ...
    // `os.urandom` draws from a libc entropy call and never from a descriptor,
    // so the name that describes this build's entropy source is the one the
    // host offers. Callers read these to decide whether descriptor semantics
    // apply at all — whether a closed or exhausted descriptor can be observed.
    // Windows draws from the cryptography provider, which neither name
    // describes, so it publishes neither.
    #[cfg(target_os = "linux")]
