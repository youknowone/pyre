...
pub fn init(ns: &mut PyNamespace) {
    ...
    namespace_store(
        ns,
        "isfinite",
        w_builtin_func_new("isfinite", |args| {
            Ok(w_bool_from(
    ...
    for (name, func) in [
        ...
        namespace_store(ns, name, w_builtin_func_new(name, func));
    }
}
