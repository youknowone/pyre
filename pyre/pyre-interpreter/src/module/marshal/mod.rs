...
/// `read_wtf8`'s validation, done with `rutf8::wtf8_from_bytes`.
///
...
/// admitted and a rejection is that call's `UnicodeDecodeError`, reported at
/// the position the validator stopped at, which the sink carries past the
