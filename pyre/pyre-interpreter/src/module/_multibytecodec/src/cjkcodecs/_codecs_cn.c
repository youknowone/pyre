...
/*
 * [3.14-spec] PyPy's HZ codec stores the mode through state->i, making the
 * state integer exposed by getstate native-endian.  Pinned v3.14.6
 * Modules/cjkcodecs/_codecs_cn.c uses the first state byte so the public
 * encoder/decoder state is architecture-independent.
 */
...
DECODER(gb2312)
{
    while (inleft > 0) {
        ...
        /* [3.14-spec] Pinned v3.14.6 DECODER(gb2312) rejects only the
         * lead byte here; PyPy DECODER(gb2312) returns a two-byte span. */
        else return 1;
    }
...
DECODER(gbk)
{
    while (inleft > 0) {
        ...
        /* [3.14-spec] Pinned v3.14.6 DECODER(gbk) rejects only the
         * lead byte here; PyPy DECODER(gbk) returns a two-byte span. */
        else return 1;
    ...
    while (inleft > 0) {
        ...
        if (c2 >= 0x30 && c2 <= 0x39) { /* 4 bytes seq */
            ...
            if (c  < 0x81 || c  > 0xFE ||
                c3 < 0x81 || c3 > 0xFE ||
                c4 < 0x30 || c4 > 0x39)
                /* [3.14-spec] Pinned v3.14.6 DECODER(gb18030) rejects only
                 * the lead byte; PyPy returns the four-byte candidate. */
                return 1;
            c -= 0x81;  c2 -= 0x30;
            c3 -= 0x81; c4 -= 0x30;
            ...
            /* [3.14-spec] Pinned v3.14.6 DECODER(gb18030) rejects only the
             * lead byte; PyPy returns the unmapped four-byte candidate. */
            return 1;
        }
        ...
        /* [3.14-spec] Pinned v3.14.6 DECODER(gb18030) rejects only the
         * lead byte; PyPy returns the unmapped two-byte candidate. */
        else return 1;
...
DECODER(hz)
{
    while (inleft > 0) {
        ...
        if (c == '~') {
            ...
            REQUIRE_INBUF(2)
            /* PyPy's DECODER(hz) writes this byte but then uses NEXT(2, 0),
             * so the real pypy3 drops "~~".  Pinned v3.14.6
             * Modules/cjkcodecs/_codecs_cn.c DECODER(hz) uses OUTCHAR('~'). */
            if (c2 == '~' && state->c[CN_STATE_OFFSET] == 0) {
                ...
                /* [3.14-spec] Pinned v3.14.6 DECODER(hz) rejects only '~';
                 * PyPy DECODER(hz) returns the two-byte escape. */
                return 1;
            NEXT(2, 0);
            continue;
        }
        ...
        if (state->c[CN_STATE_OFFSET] == 0) { /* ASCII mode */
            ...
            TRYMAP_DEC(gb2312, **outbuf, c, IN2) {
                ...
                /* [3.14-spec] Pinned v3.14.6 DECODER(hz) rejects only the
                 * lead byte; PyPy DECODER(hz) returns the two-byte pair. */
                return 1;
        }
    }
