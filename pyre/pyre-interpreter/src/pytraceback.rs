...
pub fn w_pytraceback_new(
    ...
    let raw = pyre_object::gc_hook::GcAllocOutcome::from_hook(raw)
        .allocated_or_abort(PYTRACEBACK_OBJECT_SIZE)
    ...
    // can spill the header to old-gen; remember that old-to-young `w_next` /
    // `w_code` / frame edge.
