/* The `PyArg_Parse*` family: the format walk, the units it converts and the
 * buffers an `es`/`et` unit owns.
 *
 * Compiled into the interpreter and exported from it, which is where
 * `pypy/module/cpyext/src/getargs.c` puts the same bodies.  Everything here is
...
static void _PyPyre_ArgError(const char *fname, const char *message)
{
    char buffer[256];
...
static void _PyPyre_ArgCount(const char *format, Py_ssize_t *total,
...
static int _PyPyre_ArgOwn(_PyPyre_ArgOwned *owned, char **buffer)
{
...
static void _PyPyre_ArgDisown(_PyPyre_ArgOwned *owned)
{
...
static int _PyPyre_ArgConvert(PyObject *arg, const char **format,
...
static void _PyPyre_ArgSkip(const char **format, va_list *va)
{
    char code = **format;
    (*format)++;
    switch (code) {
...
static int _PyPyre_VaParse(PyObject *args, PyObject *kwargs,
                           const char *format,
...
int _PyArg_CheckPositional(const char *name, Py_ssize_t given,
