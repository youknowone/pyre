/* The `%`-format engine and `PyUnicode_FromFormat` over it.  The pieces are
 * assembled as `str` objects rather than as bytes: width pads to a count of
 * characters and only the interpreter knows how many a conversion produced.
 *
 * Compiled into the interpreter and exported from it, which is where
 * `pypy/module/cpyext/src/unicodeobject.c` puts the same bodies.  Everything here is
...
/* `count` spaces, which is what a conversion narrower than its width is
   padded with. */
static PyObject *_PyPyre_Spaces(Py_ssize_t count)
{
    char chunk[64];
...
static PyObject *_PyPyre_Pad(PyObject *text, Py_ssize_t width, Py_ssize_t precision,
                             int left_align)
{
    if (text == NULL) {
        return NULL;
...
static PyObject *_PyPyre_FormatInteger(const char *start, const char *modifiers,
...
static PyObject *_PyPyre_FormatPointer(void *pointer)
{
    char rendered[64];
...
/* A type's fully qualified name, `module:qualname` when `alternate` asks for
   ...
   `__main__`, as `_PyType_GetFullyQualifiedName` does. */
static PyObject *_PyPyre_TypeName(PyTypeObject *type, int alternate)
{
    if (!alternate) {
...
static PyObject *_PyPyre_FormatOne(const char **cursor, va_list *va)
{
