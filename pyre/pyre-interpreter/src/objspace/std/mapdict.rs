...
/// Whether `obj` carries the storage supplied by `MapdictStorageMixin`
/// (`mapdict.py:748-761, 905-910`). Ordinary instances and `_random.Random`
/// keep the historical prefix. The generated tuple/int/str user layouts append
/// the mixin after their unchanged builtin base payload (`typedef.py:174-227`)
/// and are identified by their own GC type ids.
///
/// # Safety
...
unsafe fn mapdict_carrier(obj: PyObjectRef) -> MapdictCarrier {
    debug_assert!(unsafe { has_mapdict_storage(obj) });
    MapdictCarrier { obj }
}
