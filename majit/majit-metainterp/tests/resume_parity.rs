...
fn resume_py_public_encoding_uses_tagged_numbering() {
    ...
    // Header layout (resume.py ResumeDataLoopMemo.number):
    ...
    assert_eq!(encoded.rd_numb[4], 0); // jitcode_index
    assert_eq!(encoded.rd_numb[5], 123); // pc
    ...
    assert_eq!(encoded.decode().frames[0].pc, 123);
}
...
fn resume_py_compact_liveboxes_numbering() {
    ...
    // Slots start after jitcode_index and pc (`ResumeDataLoopMemo.number`).
