...
/// intbounds.py `get_integer_min(is_unsigned, byte_size)`.
...
/// intbounds.py `get_integer_max(is_unsigned, byte_size)`.
...
/// intbounds.py `print_rewrite_rule_statistics`.
///
...
/// class `Optimization.getintbound`/`setintbound` (optimizer.py).
...
impl OptIntBounds {
    ...
    /// intbounds.py `inv_arg1 = ConstInt(-i1)` parity — synthesise a
    ...
    fn postprocess_arraylen_gc(&mut self, op: &Op, ctx: &mut OptContext) {
        ...
        if let Some(bound) = bound {
            // optimizer.py setintbound(op, ...): `op` is the result box.
    ...
    /// intbounds.py optimize_INT_ADD_OVF
    ...
    /// intbounds.py optimize_GUARD_OVERFLOW
    ...
    /// intbounds.py propagate_bounds_backward
    ///
    /// ```python
    ...
    /// intbounds.py _propagate_int_is_true_or_zero (helper for
    ...
    fn propagate_bounds_backward_op(&mut self, op: &Op, ctx: &mut OptContext) {
        match op.opcode {
            ...
            // intbounds.py propagate_bounds_INT_SUB
            ...
            // intbounds.py propagate_bounds_INT_MUL
            ...
            // intbounds.py propagate_bounds_INT_LSHIFT
            OpCode::IntLshift => {
                let arg0 = op.arg(0);
                let arg1 = op.arg(1);
                let b1 = self.getintbound_arg(&arg0, ctx);
            ...
            // intbounds.py propagate_bounds_INT_RSHIFT
            OpCode::IntRshift => {
                let arg0 = op.arg(0);
            ...
            // intbounds.py propagate_bounds_UINT_RSHIFT
            OpCode::UintRshift => {
                let arg0 = op.arg(0);
                let b2 = self.getintbound_arg(&op.arg(1), ctx);
                if !b2.is_constant() {
            ...
            // intbounds.py propagate_bounds_INT_AND
            OpCode::IntAnd => {
                let arg0 = op.arg(0);
                let arg1 = op.arg(1);
                let r = self.getintbound_box(op.pos.get(), ctx);
            ...
            // intbounds.py propagate_bounds_INT_OR
            OpCode::IntOr => {
                let arg0 = op.arg(0);
                let arg1 = op.arg(1);
                let r = self.getintbound_box(op.pos.get(), ctx);
            ...
            // intbounds.py propagate_bounds_INT_XOR
            OpCode::IntXor => {
                let arg0 = op.arg(0);
                let arg1 = op.arg(1);
                let r = self.getintbound_box(op.pos.get(), ctx);
            ...
            // intbounds.py _propagate_int_is_true_or_zero +
            // propagate_bounds_INT_IS_TRUE / IS_ZERO.
            //
            // ```python
...
impl Optimization for OptIntBounds {
    ...
    /// `postprocess_CALL_PURE_I` (intbounds.py). It is load-bearing, and it
    ...
    /// intbounds.py _postprocess_guard_true_false_value parity.
    ///
    /// ```python
                    ...
                    && let Some(cd) = d.as_call_descr()
                {
                    ...
                    match ei.oopspecindex {
                        majit_ir::OopSpecIndex::IntPyDiv => {
                            if op.num_args() >= 3 {
                                // intbounds.py post_call_INT_PY_DIV.
                                let b1 = self.getintbound_arg(&op.arg(1), ctx);
                        ...
                        majit_ir::OopSpecIndex::IntPyMod if op.num_args() >= 3 => {
                            // intbounds.py post_call_INT_PY_MOD.
                            let b1 = self.getintbound_arg(&op.arg(1), ctx);
                            let b2 = self.getintbound_arg(&op.arg(2), ctx);
    ...
    fn export_arg_int_bounds(
        ...
        for &arg in args {
            ...
            // optimizer.py `setintbound`: an IntBound is never stored
            // on a Const (history.py a Const carries its value inline);
...
/// intbounds.py `class IntegerAnalysisLogger(object)`.
///
...
mod tests {
    ...
    fn test_call_int_py_div_uses_numerator_and_divisor_bounds() {
        // RPython intbounds.py post_call_INT_PY_DIV:
    ...
    fn test_call_int_py_mod_uses_mod_bound_for_negative_divisor() {
        // RPython intbounds.py post_call_INT_PY_MOD:
