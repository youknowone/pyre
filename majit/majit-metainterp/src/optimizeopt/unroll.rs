    ...
    for (op_idx, op) in p2_ops.iter().enumerate() {
        ...
        if let Some(label_idx) = current_inner_label_index {
            ...
            if !extra_live_args.is_empty() {
                let existing: majit_ir::vec_set::VecSet<OpRef> = result[label_idx]
                    .getarglist()
                    .iter()
                    .map(|a| a.to_opref())
                    .collect();
...
mod tests {
    use super::*;
    use crate::r#box::test_support::{rooted_inputarg_operand, rooted_resop_box, rooted_resop_operand};
    ...
    fn test_assemble_peeled_trace_keeps_used_box_with_inline_constant_operand() {
        ...
        let p1_ops = vec![{
            let mut op = Op::new(
                OpCode::SameAsI,
                &[rooted_resop_operand(Type::Int, 37)],
            );
        ...
        let p2_ops = vec![
            {
                let mut op = Op::new(
                    OpCode::GuardTrue,
                    &[rooted_resop_operand(Type::Int, 64)],
                );
