...
// Per-rule fire counters mirroring `OptIntAutoGenerated`'s generated
// `_rule_names_<op>` / `_rule_fired_<op>` class attributes and their
// registration in `_all_rules_fired` (autogenintrules.py:18-22, one triple
// per `optimize_INT_*`). Upstream keeps them as shared mutable class
// attributes bumped in-line by each rule and read back by
// `print_rewrite_rule_statistics` (intbounds.py:862-870). They are
...
/// `_all_rules_fired` (autogenintrules.py:18) — the registry iterated by
/// `print_rewrite_rule_statistics`, in the generated `.append` order.
...
/// `self._rule_fired_<op>[index] += 1` — bump the counter for the rule that
/// just fired. Inlined in the generated code at each rewrite's `return`.
#[inline]
