...
impl CachedField {
    ...
    fn get_entry(&self, struct_opref: OpRef) -> Option<OpRef> {
        for i in 0..self.cached_structs.len() {
            if self.cached_structs[i] == struct_opref {
                return Some(self.cached_values[i]);
            }
...
impl CachedArrayItem {
    ...
    fn get_entry(&self, array_opref: OpRef) -> Option<OpRef> {
        for i in 0..self.cached_arrays.len() {
            if self.cached_arrays[i] == array_opref {
                return Some(self.cached_values[i]);
            }
...
pub struct OptHeap {
    ...
    /// Reverse map for immutable field loads: loaded OpRef → (parent, field_idx).
    immutable_load_origins: HashMap<OpRef, (OpRef, u32)>,
}
...
impl OptHeap {
    pub fn new() -> Self {
        OptHeap {
            ...
            immutable_load_origins: HashMap::new(),
        }
    }
    ...
    fn get_or_create_cached_arrayitem(
        &mut self,
        descr_idx: u32,
        index: i64,
    ) -> &mut CachedArrayItem {
    ...
    fn cannot_alias_via_content(opref1: OpRef, opref2: OpRef, ctx: &OptContext) -> bool {
    ...
    fn known_distinct_via_immutable_loads(&self, opref1: OpRef, opref2: OpRef) -> bool {
        if let (Some(&(p1, f1)), Some(&(p2, f2))) = (
            self.immutable_load_origins.get(&opref1),
            self.immutable_load_origins.get(&opref2),
        ) {
            return p1 == p2 && f1 != f2;
        }
        false
    ...
    fn optimize_getfield(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        // Register in cache so that push's virtual SetfieldGc and
        // subsequent pop's GetfieldGcR can cancel out correctly.
        ...
        if let Some(cf) = self.field_cache.get(&field_idx) {
            if let Some(cached) = cf.get_entry(obj) {
                let cached = ctx.get_replacement(cached);
                ctx.replace_op(op.pos, cached);
                return OptimizationResult::Remove;
            }
        ...
        if matches!(op.opcode, OpCode::GetfieldGcR | OpCode::GetfieldGcPureR) {
            if op.descr.as_ref().map_or(false, |d| d.is_always_pure()) {
                self.immutable_load_origins.insert(op.pos, (obj, field_idx));
            }
        }
    ...
    fn optimize_setfield(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        // Check write-after-write first (before possible_aliasing).
        {
            ...
            } else if let Some(&cached) = cf.get_entry(obj).as_ref() {
        ...
        let skip_invalidate = self.field_cache.get(&field_idx).map_or(false, |cf| {
            if let Some((lazy_obj, _)) = &cf.lazy_set {
                self.known_distinct_via_immutable_loads(*lazy_obj, obj)
            } else {
                false
            }
        });
        let needs_force = self.field_cache.get(&field_idx).map_or(false, |cf| {
            ...
            if self.known_distinct_via_immutable_loads(lazy_obj, obj) {
                return false;
            }
        ...
        if needs_force {
            ...
            if let Some((lazy_obj, mut lazy_op)) = lazy_data {
                ...
                // 1. invalidate (skip pure, skip when known-distinct)
                if !self.immutable_field_descrs.contains(&field_idx) && !skip_invalidate {
                    self.get_or_create_cached_field(field_idx)
                        .invalidate_with_ctx(field_idx, ctx);
                }
        ...
        {
            let cf = self.get_or_create_cached_field(field_idx);
            if let Some(&cached) = cf.get_entry(obj).as_ref() {
                let cached_resolved = ctx.get_replacement(cached);
                if cached_resolved == new_value {
                    cf.lazy_set = None;
                    return OptimizationResult::Remove;
    ...
    fn optimize_getarrayitem(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        if let Some(key) = Self::arrayitem_key(op, ctx) {
            ...
            if let Some(cached) = ctx.imported_short_arrayitems.remove(&key) {
                ...
                if cai.get_entry(array).is_none() {
                    let d = ctx
                        .imported_short_arrayitem_descrs
                        .remove(&key)
                        .or_else(|| op.descr.clone());
                    cai.register(array, cached, d.as_ref());
                }
            }
            if let Some(cai) = self.array_cache.get(&(descr_idx, const_index)) {
    ...
    fn optimize_setarrayitem(&mut self, op: &Op, ctx: &mut OptContext) -> OptimizationResult {
        ...
        // Write-after-write check.
        {
            ...
            } else if let Some(cached) = cai.get_entry(array) {
                let cached = ctx.get_replacement(cached);
                if cached == new_value {
                    return OptimizationResult::Remove;
                }
...
impl Optimization for OptHeap {
    ...
    fn setup(&mut self) {
        ...
        self.cached_arrayitems_var.clear();
        self.array_min_lengths.clear();
        self.immutable_load_origins.clear();
    }
