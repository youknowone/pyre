...
impl<S: JitState> JitDriver<S> {
    ...
    fn back_edge_internal(
        ...
        if self.meta.has_compiled_loop(green_key) {
            ...
            // resume pc.
            if should_bridge {
                // resume.py:924/993 `_prepare(storage)` parity: apply the
                // guard's pending heap writes (`rd_pendingfields`) to the live
                // storage BEFORE reconstructing state. The blackhole resume
                // path replays these (via `blackhole_from_resumedata`), so a
                // bridge that does not replay them traces from a heap that
                // disagrees with blackhole — e.g. a list's `.size` field left
                // at its stale pre-write value, making the bridge's
                // `selected_ref.size` re-sync read a too-small `stacksize` and
                // record the wrong loop-control branch. Both PyPy rebuild paths
                // (`rebuild_from_resumedata`, `blackhole_from_resumedata`) share
                // `_prepare`; mirror that here before recovering reds.
