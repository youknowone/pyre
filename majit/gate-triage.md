...
| `MAJIT_CL_BRIDGE_MERGE_LOG` | OFF | Prints one line per merge attempt — merged, or the reason it was declined — for the owner-loop merge; remove it with that merge. |
| `MAJIT_CL_NO_BRIDGE_MERGE` | OFF | Disables Cranelift's merge of a loop-closing bridge into its owner function, restoring the out-of-line recovery stub; remove when the out-of-line route no longer needs a comparison path. |
