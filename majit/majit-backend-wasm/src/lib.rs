...
impl WasmBackend {
    ...
    fn reemit_loop_retiring(
        ...
        let merged_ref_homes = module_data.num_ref_homes;
        let merged_labels = module_data.used_label_homes;
...
pub fn dead_frame_from_ran_frame(_compiled_ptr: usize, frame_ptr: usize) -> DeadFrame {
    ...
    let jf = jitframe_object_base(frame_ptr).0 as *mut majit_backend::jitframe::JitFrame;
...
fn dead_frame_from_forced_frame(frame_ptr: usize, fail_index: u32) -> DeadFrame {
    ...
    // incoming items pointer.
    data.attach_origin_jf(
        jitframe_object_base(frame_ptr).0 as *mut majit_backend::jitframe::JitFrame,
    );
    DeadFrame::Boxed(data)
}
...
impl majit_backend::Backend for WasmBackend {
    ...
    fn compile_loop(
        ...
        let (wasm_bytes, guard_exits, module_data) =
        ...
        let num_ref_homes = module_data.num_ref_homes;
        ...
        let compiled = CompiledWasmLoop {
            ...
            max_output_slots,
            num_ref_homes: std::cell::Cell::new(module_data.num_ref_homes),
            used_label_homes: std::cell::Cell::new(
                used_label_homes.max(module_data.used_label_homes),
            ),
            frame,
    ...
    fn execute_token(&self, token: &JitCellToken, args: &[Value]) -> DeadFrame {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            if wasm_jitframe_tid() != 0 {
                ...
                // barrier now; the drop is after `seed_savedata_from_jf`.
                wasm_jit_write_barrier(jf as i64);
                let mut data = WasmFrameData::boxed(raw_values, fail_descr, exc_value);
