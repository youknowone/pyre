    ...
    let fail_descrs: Vec<_> = guards
        ...
        .map(|(guard, op)| {
            ...
            Arc::new(crate::failguard::WasmFailDescr {
                ...
                fail_locs: guard.fail_locs.clone(),
                rd_locs,
    ...
    for (op_idx, op) in ops.iter().enumerate() {
        ...
        match op.opcode {
            ...
            opcode if opcode.is_call_assembler() && ca.emit_ca => {
                ...
                // assembler.py `_reload_frame_if_necessary` then reapplies
                // the non-array write barrier after the reload.
                if ca.inline.is_none() {
