    ...
    let (flow_result, narrow_root, remove_upto) = if ci + 1 == ops_len {
        ...
        match (&cast.kind, cast.result.as_ref()) {
            (
                ...
                && segments[0] == crate::pyre_names::shims::CAST_INSTANCE
                && args.len() == 1
        ...
        let narrowed = g
            .push_op_var(
                ...
                OpKind::Call {
                    target: CallTarget::FunctionPath {
                        segments: vec![
                            crate::pyre_names::shims::CAST_INSTANCE.into(),
                            "PyObject".into(),
                        ],
                    },
                    args: vec![opt.clone()],
        ...
        let narrow_casts = g
            ...
            .filter(|op| {
                matches!(
                    &op.kind,
                    OpKind::Call { target: CallTarget::FunctionPath { segments }, .. }
                        if segments.first().map(String::as_str) == Some(crate::pyre_names::shims::CAST_INSTANCE)
                )
            })
            .count();
