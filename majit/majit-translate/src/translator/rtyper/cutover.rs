...
///
/// The returned tag identifies the matching fallback condition. Keeping the
/// category and the boolean decision in one function prevents diagnostic code
...
/// one list of fallback conditions.
...
/// Surgical per-graph re-drive for the two-pass builder lift gate (design B).
///
/// Every `paths` slot in `function_graphs` has already been swapped from its
/// `ll_strconcat` graph to the `StringBuilder` variant.  This re-drives each
/// swapped subject against the SHARED prepass session — annotate-half (Phase A)
/// then rtype (Phase B) — and overwrites its cached [`TwoPhaseSubject`] with the
/// builder-typed result so the drain publishes the variant.
///
/// Only the swapped graphs are driven — each a NEW subject with fresh flowspace
/// Variables.  A whole-prepass re-run instead re-drives every already-rtyped
/// concat graph, which re-seeds a cached rtyped PyGraph into the per-session
/// annotator and trips the `unimplemented operation: 'int_add'` /
/// monotonicity asserts (`drive_subject` documents this); driving only the new
/// variants avoids that.
///
/// The concat subject for every path is saved and restored for any path whose
/// variant fails to re-lift (annotate or rtype failure), so a failed swap
/// degrades to the concat graph the prepass already proved lifts.  Returns the
/// failed paths — the caller swaps their `function_graphs` slot back to concat.
pub(crate) fn redrive_builder_variant_subjects(
    call_registry: &PyreCallRegistry,
    function_graphs: &crate::codewriter::call::GraphStore,
    paths: &[crate::parse::CallPath],
) -> Vec<crate::parse::CallPath> {
    use crate::translator::rtyper::pyre_call_registry::TwoPhaseSubject;

    // Deterministic drive order (R3): the caller iterates the side-map in
    // HashMap order; sort so classdef numbering for the variants' new reprs is
    // run-to-run stable, matching `run_two_phase_prepass_inner`.
    let mut ordered: Vec<&crate::parse::CallPath> = paths.iter().collect();
    ordered.sort_by_key(|p| p.canonical_key());

    let mut saved: Vec<(String, Option<TwoPhaseSubject>)> = Vec::new();
    let mut driven_ok: Vec<&crate::parse::CallPath> = Vec::new();
    let mut failed: Vec<crate::parse::CallPath> = Vec::new();

    // ── Phase A — re-annotate each swapped variant ───────────────────
    for path in &ordered {
        let path = *path;
        let key = path.canonical_key();
        let Some(variant) = function_graphs.get(path) else {
            failed.push(path.clone());
            continue;
        };
        let session_at_entry = SubjectSessionSnapshot::capture(call_registry);
        let attempt = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            drive_subject(variant, call_registry, /* do_rtype = */ false)
        }));
        // Take the concat subject out; keep it for restore on failure.
        let old = call_registry.two_phase().subjects.remove(&key);
        saved.push((key.clone(), old));
        match attempt {
            ...
            Ok(Err(_)) | Err(_) => {
                failed.push(path.clone());
                // Repair any shared-callee state the failed annotate half
                // touched, exactly as Phase A does (`run_two_phase_prepass_inner`),
                // so a later variant drive is not poisoned by this one.
                let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
    ...
    // ── inheritance-id barrier + return-var seeding (mirror inner) ────
    ...
    // ── Phase B — rtype the driven variants ──────────────────────────
    run_phase_b_rtype_isolated(
        call_registry,
        function_graphs,
        /* determinism_trace = */ false,
        &driven_ok,
    );

    // A variant that annotated but failed to rtype records its flowspace
    // graph_key in `rtype_skipped`; treat it as failed so it reverts to concat.
    for path in &driven_ok {
        let key = path.canonical_key();
        let skipped = {
            let tp = call_registry.two_phase();
            match tp.subjects.get(&key) {
                Some(subject) => tp.rtype_skipped.contains(&subject.graph_key),
                None => true,
            }
        };
        if skipped {
    ...
    // ── restore concat subjects for every failed path ────────────────
    let failed_keys: std::collections::HashSet<String> =
        failed.iter().map(|p| p.canonical_key()).collect();
    for (key, old) in saved {
        if failed_keys.contains(&key) {
            let mut tp = call_registry.two_phase();
            match old {
                Some(subject) => {
                    tp.subjects.insert(key, subject);
                }
                None => {
                    tp.subjects.remove(&key);
                }
            }
        }
    }
...
mod tests {
    ...
    fn known_unported_classifies_indirect_call_adapter_invariant() {
        ...
        assert_eq!(
            ...
            "the indirect-call gap keeps its stable fallback category"
        );
    ...
    fn known_unported_classifies_noneify_on_ptr_merge() {
        ...
        assert_eq!(
            unported_category(msg),
            Some("noneify-on-ptr"),
            "the typed-null-pointer gap keeps its stable fallback category"
        );
        assert!(is_known_unported(msg));
    }

    #[test]
    fn unknown_rtyper_failure_is_not_skip_classified() {
