...
/// Classdef-less `Ref` shells are not counted in a thread-local.
/// A `SomeInstance` carries its `classdef` only when the caller has a
/// `Bookkeeper` (`Bookkeeper::getuniqueclassdef`). This projector does not.
