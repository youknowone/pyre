...
fn bracket_closes_in_the_crate_that_owns_the_guard() {
    ...
    assert_bracket_closes(llbc, "w_list_append");
}
...
/// A bracket left open must belong to a guard the body moved out of.
///
...
#[test]
fn only_a_moved_out_guard_keeps_its_bracket_open() {
    let Some(llbc) = object_llbc() else { return };
    ...
    for fd in llbc.iter_local_fns() {
        ...
        assert!(
            guard_drop_sites(llbc, &body).any(|(_, local)| moved.contains(&local)),
            "{name} drops a root-bracket guard, lowers no close for it, and never \
             moves the guard out: every slot the bracket pins stays reachable for \
    ...
    assert!(
        !left_open.is_empty(),
        "no function left a bracket open, so the moved-out-guard case has no live \
         fixture and this test asserted nothing"
    );
}
...
/// Most of the brackets this crate drops must actually close.
///
...
fn nearly_every_dropped_bracket_closes() {
    ...
    assert!(
        bodies > 100,
        "only {bodies} bodies drop a guard; the artefact looks wrong, so a pass \
         here would prove nothing"
    );
    assert!(
...
/// The closes a body owes: one per drop of an unmoved guard, counting only the
/// drops the front lowers. A cleanup-only drop is not one of them — the front
/// does not follow `on_unwind`.
fn owed_closes(llbc: &Llbc, body: &Unstructured) -> usize {
    let moved = moved_out_locals(body);
    let live = reachable_without_unwind(body);
    guard_drop_sites(llbc, body)
        .filter(|(bb, local)| live[*bb] && !moved.contains(local))
        .count()
}
