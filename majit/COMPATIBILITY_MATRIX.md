...
| Static analyzer / translator | `rpython/jit/codewriter/*` + translation-time front-end | `partial` | `majit-analyze` parses multiple Rust source files, extracts opcode dispatch arms, resolves cross-file trait impls and helper classifications, collects type layouts, and generates tracing helper code. Consumed by `pyre-mjit` build.rs. Classifies 39/40 pyre opcode arms (arithmetic, comparisons, local/const access, jumps, returns, namespace ops, iterator cleanup, stack manipulation, function calls, noop). Complex CFG (loop/while/for in match arms) generates abort JitCode for interpreter fallback matching RPython dont_look_inside semantics. Only the wildcard `_` arm remains unclassified. Remaining parity gap: the current analyzer still relies heavily on source-string heuristics instead of a graph-based semantic translator like `rpython/jit/codewriter/`. |
...
Most subsystems are `implemented`, but the analyzer / translator remains `partial`.

Main remaining refinement areas:
...
- **Static analyzer architecture**: `majit-analyze` still uses source-string heuristics (`quote!(...).to_string()`, `contains(...)`) in extraction and classification. The long-term target is a narrow semantic graph plus graph rewrite passes, matching the role of `rpython/jit/codewriter/` more closely.
