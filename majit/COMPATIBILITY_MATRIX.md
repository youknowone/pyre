...
8 subsystems are `implemented`, 1 remains `partial`.

Remaining gap:

1. **Cranelift backend**: frame-stack metadata emitted and consumed for 3-4 level nested restores, but RPython resume.py level arbitrary caller-stack reconstruction is not fully exercised under real compiled interpreter stacks. This is the last structural gap before full RPython JIT parity.
