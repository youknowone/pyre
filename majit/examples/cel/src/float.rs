//! Float register-machine benchmark for the macro-generated JIT path.
//!
...
//!   ACC: sum(a[i]), one load and one floating-point add.
//!   DOT: sum(a[i] * b[i]), two loads, one multiply, and one add.
...
mod twobank {
    ...
    ///
    /// `R_BOOL` implements `acc += a[i] >= b[i]`. `R_ONE` instead increments on
    /// every row, which makes duplicated or skipped iterations observable even
    /// when the affected row's comparison is false.
