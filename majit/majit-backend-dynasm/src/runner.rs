...
/// llmodel.py:495-497 `bh_setarrayitem_gc_r` barrier: the generic
/// `do_write_barrier(gcref, newvalue)`. The blackhole interpreter is not
/// the JIT, so no inline TRACK_YOUNG_PTRS test precedes the call.
/// `is_managed_heap_object` first guards the pyre-specific case where a
/// reconstructed frame array slot is not a GC-managed object (RPython's
/// frame arrays always are), then `write_barrier` flag-checks. Mirrors
/// cranelift's `write_barrier_if_managed`.
