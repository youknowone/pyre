...
- broader proc-macro-to-driver parity is still incomplete: multi-branch control-flow graphs and fully automatic whole-module call discovery do not exist, `#[jit_inline]` only covers simple int helpers, ref/float helper return signatures are unsupported, and full declarative driver lifecycle parity is missing
