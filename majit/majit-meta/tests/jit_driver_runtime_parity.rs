...
#[test]
fn generic_back_edge_restores_and_syncs_virtualizable_state() {
    let descriptor = AutoVirtualizableDriver::descriptor(
        &[Type::Int, Type::Int],
        &[Type::Ref, Type::Int, Type::Int],
    )
    ...
    let green_values = [444_i64, 444_i64];
    let key_hash = majit_meta::green_key_hash(&green_values);

    let run_generic = |driver: &mut JitDriver<AutoVirtualizableState>,
                       state: &mut AutoVirtualizableState|
     -> Option<usize> {
        if !driver.is_tracing() && !driver.has_compiled_loop(key_hash) {
            if !driver
                .meta_interp_mut()
                .warm_state_mut()
                .counter_tick_checked(key_hash)
            {
                return None;
            }
        }
    ...
    state.stackpos = -10;
    state.top = -10;
    assert_eq!(run_generic(&mut driver, &mut state), Some(444));
    assert_eq!(state.stackpos, 8);
    assert_eq!(state.top, 42);
    assert_eq!(frame.stackpos, 8);
    assert_eq!(stack[0], 42);
