...
def test_abi3_tags_for_pyre_interpreter(fake_packaging):
    tags = _import(fake_packaging)
    for interpreter in ['pyre%d' % sys.version_info[0],
                        'pyre%d%d' % sys.version_info[:2]]:
        result = list(tags.compatible_tags(interpreter=interpreter,
                                           platforms=['p']))
        abi3 = [t for t in result if t.abi == 'abi3']
        assert abi3, interpreter
        assert all(t.interpreter.startswith('cp3') for t in abi3)
